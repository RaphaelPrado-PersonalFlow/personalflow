...
export default function OpenProtocolPage({ params }: Props) {
  ...
  return <MainLayout><div className="space-y-6">
    <PageHeader title={protocol.name ?? protocol.objective} description={`${protocol.student} · ${protocol.objective} · ${protocol.frequency}× por semana`} action={<div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => router.push(`/treinos/${studentId}`)}>← Protocolos</Button><Button onClick={() => router.push(editorUrl(workouts[0]?.id))}>Editar protocolo</Button></div>} />
