...
export default function StudentsPage() {
  ...
  async function addStudent(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name || !phone || !email || !goal) return;
    setSaving(true);
    setFeedback("");
    try {
      const created = await createStudent({ full_name: name, phone, email, cpf: cpf || null, goal });
  ...
  async function editStudent(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    const cpf = String(data.get("cpf") || "").trim();
    const goal = String(data.get("goal") || "").trim();
    if (!fullName || !phone || !email || !goal) return;
    setSaving(true);
    setFeedback("");
    try {
      ...
      setFeedback("Dados do aluno atualizados.");
    } catch {
      setFeedback("Não foi possível atualizar o aluno. Confira os dados e tente novamente.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <MainLayout>
      ...
      {modalOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/75 p-4" role="dialog" aria-modal="true"><form onSubmit={addStudent} className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-2xl"><div className="flex items-center justify-between"><div><h2 className="text-xl font-semibold">Novo aluno</h2><p className="mt-1 text-sm text-[var(--muted)]">Dados essenciais para cadastro e futuro acesso do aluno.</p></div><button type="button" onClick={() => setModalOpen(false)} className="grid size-9 place-items-center rounded-lg">×</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium sm:col-span-2">Nome completo<input name="name" required placeholder="Nome do aluno" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium">Telefone<input name="phone" required placeholder="(21) 99999-9999" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium">CPF (opcional)<input name="cpf" placeholder="000.000.000-00" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium sm:col-span-2">E-mail<input name="email" required type="email" placeholder="aluno@email.com" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium sm:col-span-2">Objetivo principal<select name="goal" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3"><option>Hipertrofia</option><option>Emagrecimento</option><option>Força</option><option>Condicionamento</option><option>Qualidade de vida</option></select></label></div><div className="mt-6 flex justify-end gap-3"><Button type="button" variant="ghost" onClick={() => setModalOpen(false)}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? "Salvando..." : "Cadastrar aluno"}</Button></div></form></div>}

      {studentToEdit && <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/75 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-student-title"><form onSubmit={editStudent} className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-2xl"><div className="flex items-center justify-between"><div><h2 id="edit-student-title" className="text-xl font-semibold">Editar aluno</h2><p className="mt-1 text-sm text-[var(--muted)]">Atualize os dados cadastrais do aluno.</p></div><button type="button" onClick={() => setStudentToEdit(null)} className="grid size-9 place-items-center rounded-lg">×</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium sm:col-span-2">Nome completo<input name="name" required defaultValue={studentToEdit.name} className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium">Telefone<input name="phone" required defaultValue={studentToEdit.phone} className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium">CPF (opcional)<input name="cpf" defaultValue={studentToEdit.cpf} placeholder="000.000.000-00" className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium sm:col-span-2">E-mail<input name="email" required type="email" defaultValue={studentToEdit.email} className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label><label className="block text-sm font-medium sm:col-span-2">Objetivo principal<input name="goal" required defaultValue={studentToEdit.goal} className="mt-2 h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3" /></label></div><div className="mt-6 flex justify-end gap-3"><Button type="button" variant="ghost" onClick={() => setStudentToEdit(null)}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? "Salvando..." : "Salvar alterações"}</Button></div></form></div>}

      {selectedStudent && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/70"><button className="flex-1" onClick={() => setSelectedStudent(null)} aria-label="Fechar perfil" /><aside className="h-full w-full max-w-md overflow-y-auto border-l border-[var(--border)] bg-[var(--surface)] p-6"><div className="flex justify-between"><h2 className="text-xl font-semibold">Perfil do aluno</h2><button onClick={() => setSelectedStudent(null)}>×</button></div><div className="mt-8 text-center"><div className="mx-auto grid size-20 place-items-center rounded-full bg-blue-500/10 text-xl font-semibold text-blue-500">{selectedStudent.initials}</div><h3 className="mt-4 text-xl font-semibold">{selectedStudent.name}</h3><p className="text-sm text-[var(--muted)]">{selectedStudent.goal}</p></div><div className="mt-6 grid grid-cols-2 gap-2"><a href={`/treinos/${selectedStudent.id}`} className="inline-flex h-11 items-center justify-center rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white">Treinos</a><a href={`/avaliacoes?aluno=${encodeURIComponent(selectedStudent.name)}`} className="inline-flex h-11 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--surface-raised)] px-4 text-sm font-semibold">Avaliação física</a></div><Button variant="secondary" className="mt-3 w-full" onClick={() => setStudentToEdit(selectedStudent)}>Editar dados</Button><div className="mt-8 space-y-3"><Card><p className="text-xs text-[var(--muted)]">Telefone</p><p className="mt-1 font-medium">{selectedStudent.phone}</p></Card><Card><p className="text-xs text-[var(--muted)]">E-mail</p><p className="mt-1 font-medium">{selectedStudent.email}</p></Card><Card><p className="text-xs text-[var(--muted)]">CPF</p><p className="mt-1 font-medium">{selectedStudent.cpf || "Não informado"}</p></Card><Card><label className="text-xs font-medium text-[var(--muted)]" htmlFor="student-observations">Observações do aluno</label><textarea id="student-observations" value={selectedStudent.observations} onChange={(event) => { const observations = event.target.value; setSelectedStudent((student) => student ? { ...student, observations } : null); setStudents((current) => current.map((student) => student.id === selectedStudent.id ? { ...student, observations } : student)); }} onBlur={(event) => updateStudentNotes(selectedStudent.id, event.target.value).catch(() => setFeedback("Não foi possível salvar as observações."))} placeholder="Registre informações importantes para os próximos atendimentos." rows={5} className="mt-2 w-full resize-y rounded-xl border border-[var(--border)] bg-[var(--background)] p-3 text-sm outline-none focus:border-blue-500" /></Card></div><div className="mt-8 border-t border-[var(--border)] pt-6"><Button variant="secondary" className="w-full border-red-500/30 text-red-600 hover:bg-red-500/10" disabled={deleting} onClick={removeSelectedStudent}>{deleting ? "Excluindo..." : "Excluir aluno"}</Button><p className="mt-2 text-center text-xs text-[var(--muted)]">Esta ação remove o cadastro do aluno definitivamente.</p></div></aside></div>}
    </MainLayout>
  );
}
