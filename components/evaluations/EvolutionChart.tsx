      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="mx-auto block h-auto w-full max-w-[600px]" role="img" aria-label="Gráfico de evolução das medidas selecionadas">
        ...
        {series.map(({ metric, points }, seriesIndex) => {
          ...
          return <g key={metric.key}>{points.length > 1 && <path d={path} fill="none" stroke={metric.color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />}{points.map((point) => {
            ...
            const labelOffsetY = multipleSeries && seriesIndex > 0 ? 23 : -11;
            return <g key={`${metric.key}-${point.index}`}><circle cx={x(point.index)} cy={y(point.value)} r="5" fill={metric.color} stroke="var(--surface)" strokeWidth="3" /><text x={x(point.index) + labelOffsetX} y={y(point.value) + labelOffsetY} fill={metric.color} fontSize="16" fontWeight="800" textAnchor="middle" stroke="var(--background)" strokeWidth="4" paintOrder="stroke">{point.value.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}</text></g>;
          })}</g>;
        })}
      </svg>
    </div>
