...
type Props = {
  ...
  onUpdateSeriesEffort: (id: string, seriesIndex: number, field: "actual_rir" | "actual_rpe", value: string) => void;
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4 sm:p-5">
        {props.exercises.map((exercise, index) => {
          ...
          return <article key={exercise.id} className={`rounded-2xl border p-4 ${done ? "border-emerald-500/40 bg-emerald-500/10" : exercise.changed ? "border-amber-500/40 bg-amber-500/5" : "border-[var(--border)] bg-[var(--background)]"}`}>
            ...
            </div><div className="mt-2 grid grid-cols-2 gap-2"><label className="text-[10px] text-[var(--muted)]">RIR<input inputMode="decimal" defaultValue={item.actualRir ?? ""} onBlur={(event) => props.onUpdateSeriesEffort(exercise.id, seriesIndex, "actual_rir", event.target.value)} className="mt-1 h-8 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 text-xs" /></label><label className="text-[10px] text-[var(--muted)]">RPE<input inputMode="decimal" defaultValue={item.actualRpe ?? ""} onBlur={(event) => props.onUpdateSeriesEffort(exercise.id, seriesIndex, "actual_rpe", event.target.value)} className="mt-1 h-8 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 text-xs" /></label></div><div className="mt-2 flex gap-1"><button type="button" onClick={() => props.onUpdateSeriesStatus(exercise.id, seriesIndex, "completed")} className="rounded-md border border-emerald-500/30 px-2 py-1 text-[10px] font-semibold text-emerald-600">Concluída</button><button type="button" onClick={() => props.onUpdateSeriesStatus(exercise.id, seriesIndex, "partial")} className="rounded-md border border-amber-500/30 px-2 py-1 text-[10px] font-semibold text-amber-600">Parcial</button><button type="button" onClick={() => props.onUpdateSeriesStatus(exercise.id, seriesIndex, "skipped")} className="rounded-md border border-slate-500/30 px-2 py-1 text-[10px] font-semibold text-[var(--muted)]">Pular</button></div></div>)}</div>
