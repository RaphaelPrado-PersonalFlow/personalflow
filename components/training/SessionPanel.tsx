        ...
        {props.exercises.map((exercise, index) => {
          ...
          return <article key={exercise.id} className={`rounded-2xl border p-4 ${done ? "border-emerald-500/40 bg-emerald-500/10" : exercise.changed ? "border-amber-500/40 bg-amber-500/5" : "border-[var(--border)] bg-[var(--background)]"}`}>
            <div className="flex items-start gap-3">
              ...
              <button type="button" onClick={() => toggle(exercise.id)} className="min-w-0 flex-1 text-left"><span className="flex items-center gap-2"><strong className="block truncate">{exercise.name}</strong>{exercise.changed && <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-semibold text-amber-600">Alterado</span>}</span><span className="mt-1 block text-sm text-[var(--muted)]">{exercise.prescription} · {exercise.load}</span></button>
            ...
            {isExpanded && <div className="mt-3 space-y-2 border-t border-[var(--border)] pt-3">{series.map((item, seriesIndex) => <div key={seriesIndex} className="rounded-xl bg-[var(--surface)] p-3"><div className="flex justify-between gap-2"><strong className="text-xs">{label(item, seriesIndex)}</strong><span className="text-[10px] text-[var(--muted)]">Ajustes desta série</span></div><div className="mt-2 grid grid-cols-2 gap-2">
