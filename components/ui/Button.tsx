...
export default function Button({ variant = "primary", className = "", disabled, ...props }: Props) {
  ...
  const finishSessionAction = props.children === "Finalizar sessão";
  return <button disabled={finishSessionAction ? false : disabled} className={`inline-flex h-10 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition disabled:opacity-50 ${styles[variant]} ${className}`} {...props} />;
}
