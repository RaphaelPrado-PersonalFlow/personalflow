-- Keep the current period slot while copy-on-write creates the next workout version.
-- Historical sessions continue referencing the immutable source workout directly.
...
begin
  ...
  insert into public.workouts (id,professional_id,period_id,lineage_id,version,supersedes_workout_id,is_current,published_at,name,focus,sequence,estimated_duration_minutes,target_executions,notes,status)
    select new_workout_id,professional_id,period_id,lineage_id,next_version,id,true,now(),name,focus,sequence,estimated_duration_minutes,target_executions,notes,'active' from public.workouts where id = source_workout.id;

  -- The slot represents the current period schedule. Moving it avoids violating its
  -- unique period/sequence constraint and keeps it pointing at the new current version.
  update public.period_workout_slots set workout_id = new_workout_id
    where workout_id = source_workout.id and professional_id = owner_id;
